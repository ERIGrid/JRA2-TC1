/// Modified from original version to comply with different simulation server.
/// (c)  2017-2018 VTT Technical Research Centre of Finland Ltd. All rights reserved.
///  Author: Matti Laukkanen (matti.laukkanen@vtt.fi)
        ...
        int bytes, char * error )
{
    ...
    packet = (char *)malloc(pkt_size);
    {
        ...
        if( bytes > 0 )
        {
            ...
            memcpy( packet+hdr_size, data, bytes );
        }
        ...
        double * data, int * length )
{
	...
	char *request = (char*)malloc(8 * sizeof(char));
	sprintf(request, "RST%d", *length);
    char address[16];
    char err[128];
    int sd;
...
void pscad_recv_( int * ip1, int * ip2, int * ip3, int * ip4, int * port,
        double * data, int * length )
{
	char *request = (char*)malloc(7 * sizeof(char));
	sprintf(request, "GET%d", *length);
